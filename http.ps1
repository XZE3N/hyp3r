...
$modulename = "$env:USERPROFILE\AppData\Local\Microsoft\hyp3r\psscript.ps1"
		...
		Write-Output "	`$localip` = (Get-WmiObject -Class Win32_NetworkAdapterConfiguration | Where-Object {`$_`.DHCPEnabled -ne `$null` -and `$_`.DefaultIPGateway -ne `$null`}).IPAddress" >> $modulename
...
while($true) {
	...
	$localip = (Get-WmiObject -Class Win32_NetworkAdapterConfiguration | Where-Object {$_.DHCPEnabled -ne $null -and $_.DefaultIPGateway -ne $null}).IPAddress
